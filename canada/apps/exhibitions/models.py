...
class Exhibition(models.Model):
    ...
    frontpage = models.BooleanField(
        ...
        help_text="Checking will use this exhibition for the front page. \
                   To change to a different exhibition, \
                   simply enable this on that exhibition"
       )
    ...
    frontpage_selected_image = models.ForeignKey(
        ...
        help_text='Uploaded image will take preference. \
                   Clear the uploaded image to use the selected image.',
        blank=True,
        null=True,
       )
    frontpage_text = models.TextField(
        ...
        help_text="""Will be added underneath exhibition info<br>
                    ...
                    <a href="http://en.wikipedia.org/wiki/Markdown#Syntax_exam\
                    ples" target="_blank">Wikipedia entry on Markdown</a>
                    or the <a href="http://daringfireball.net/projects/markdow\
                    n/basics" target="_blank">Markdown reference</a></i>""",
        blank=True,
        null=True,
       )
    ...
    def clean(self):
        if self.frontpage:
            if not self.frontpage_image():
                raise ValidationError("Either upload or specify\
                                       a frontpage image.")
            elif not Exhibition.objects.exclude(pk=self.pk).filter(frontpage=True).exists():
                raise ValidationError("Enable a different exhibition \
                                      to change the frontpage.")

        if not self.start_date < self.end_date:
            raise ValidationError("The start date can not be after\
                                   the end date")
